...
public class Blackjack
{
    ...
    public Blackjack()
    {
        ...
        for ( int i = 0; i < NUMBER_OF_PLAYERS; i++ )
        {
            if ( i != humanIndex )
            {
                random = (int)( Math.random() * nameList.size() );
                String tempName = nameList.get( random );
                while ( tempName.compareToIgnoreCase( name ) == 0 )
                {
                    random = (int)( Math.random() * nameList.size() );
                    tempName = nameList.get( random );
                }
                // NAME[random] = name;
     ...
     * 
     * @param p
     *            Player with the hand to be printed.
     ...
     * 
     * @param whoseTurn
     *            Turn of player to be introduced
     * @param dealer
     *            Dealer for the round
    ...
    {
        int r = (int)( 5 * Math.random() ) + skill[whoseTurn];
        if ( dealer == whoseTurn )
        {
        ...
        {
            love.peekingStrategy( players[whoseTurn], players[dealer], deck1 );
        }
        else
        {
    ...
    private boolean cheat()
    {
        window.message( "The card in your sleeve is "
            ...
            + ". You can cheat by swapping this card with your "
            + players[humanIndex].getHand().get( 0 ).toString() + "." );
    ...
    /**
     * Returns a list of the players within the game, starting with the human
     * player.
     * 
    ...
    /**
     * Changes the value of the string "input". This method simulates a button
     * press.
     * 
     * @param change
     *            The string input is to be changed to.
     */
